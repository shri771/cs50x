...
// preferences[i][j] is number of voters who prefer i over j(menas the differec
// between i-j)
int preferences[MAX][MAX];

// locked[i][j] means i is locked in over j(means if true then i points toward
// j)
bool locked[MAX][MAX];
...
int main(int argc, string argv[]) {
  ...
  for (int i = 0; i < voter_count; i++) {
    ...
    for (int j = 0; j < candidate_count; j++) {
      ...
      // check for invalid vote and populate the ranks array
...
// Test for cycle by checking arrow coming into each can
bool has_cycle(int end, int cycle_start) {
